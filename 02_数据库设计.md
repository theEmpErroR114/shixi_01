...
erDiagram
    ...
    t_exam_record {
        BIGINT exam_record_id PK "测验记录ID"
        BIGINT student_id FK "学生ID"
        BIGINT paper_id FK "试卷ID"
    ...
    t_exam_answer {
        ...
        BIGINT exam_record_id FK "所属测验记录ID"
        BIGINT question_id FK "习题ID"
        VARCHAR student_answer "学生作答内容"
        TINYINT is_correct "1正确 0错误"
        DECIMAL score "该题得分"
...
> - `teacher_id` 在 `t_question` 和 `t_paper` 中为展示字段，**无外键约束**，删除老师不影响其题目和试卷
> - `student_id` 在 `t_practice_record` 和 `t_exam_record` 中设置了 `ON DELETE SET NULL`，删除学生后保留记录
> - `t_teacher_course` 和 `t_student_course` 为课程分配的多对多关联表，与 `schema.sql` 保持一致（本文档原为11表，实际运行数据库为**13表**）
